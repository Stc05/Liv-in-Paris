...
namespace Liv_in_Paris
{
    public class Graph
    {
        ...
        /// <summary>
        /// Créer un dictionnaire dont la clé est lindice de la station et la valeur le noeud initialisé à partir d'un fichier excel pour faciliter l'accès à la référence d'un noeud à partir de son id
        /// </summary>
        ...
        /// <summary>
        /// Affiche un chemin entre plusieurs stations de métro sur une carte avec les informations sur chaque station et le temps estimé de trajet entre elles.
        /// Le titre de la visualisation est affiché en haut de l'image.
        /// </summary>
        /// <param name="graph">Le graphe représentant les stations de métro et leurs connexions</param>
        /// <param name="chemin">La liste des noeuds représentant le chemin entre les stations</param>
        /// <param name="titre">Le titre à afficher en haut de la carte, généralement le plat associé à chaque chemin</param>

        public void VisualiserCheminSurCarte(Graph graph, List<Noeud> chemin, string titre)
        {
            int width = 1200;
            int height = 800;
            ...
            double minLat = chemin.Min(n => n.Latitude);
            double maxLat = chemin.Max(n => n.Latitude);
            double minLon = chemin.Min(n => n.Longitude);
            double maxLon = chemin.Max(n => n.Longitude);

            Func<double, double, double, int, int> scale = (val, min, max, size) =>
                (int)((val - min) / (max - min) * (size - 100)) + 50;

            Dictionary<int, SKPoint> positions = new();

            foreach (var noeud in chemin)
            {
                int x = scale(noeud.Longitude, minLon, maxLon, width);
                int y = scale(maxLat - noeud.Latitude, 0, maxLat - minLat, height);
                positions[noeud.Id] = new SKPoint(x, y);
            }

            Dictionary<string, SKPaint> couleurParLigne = new()
            {
                { "1", new SKPaint { Color = SKColors.Gold, StrokeWidth = 3 } },
                { "2", new SKPaint { Color = SKColors.DeepSkyBlue, StrokeWidth = 3 } },
                { "3", new SKPaint { Color = SKColors.Orange, StrokeWidth = 3 } },
                { "4", new SKPaint { Color = SKColors.Violet, StrokeWidth = 3 } },
                { "5", new SKPaint { Color = SKColors.Brown, StrokeWidth = 3 } },
                { "6", new SKPaint { Color = SKColors.LightGreen, StrokeWidth = 3 } },
                { "7", new SKPaint { Color = SKColors.Purple, StrokeWidth = 3 } },
                { "8", new SKPaint { Color = SKColors.Pink, StrokeWidth = 3 } },
                { "9", new SKPaint { Color = SKColors.DarkOliveGreen, StrokeWidth = 3 } },
                { "10", new SKPaint { Color = SKColors.DarkTurquoise, StrokeWidth = 3 } },
                { "11", new SKPaint { Color = SKColors.SaddleBrown, StrokeWidth = 3 } },
                { "12", new SKPaint { Color = SKColors.DarkSeaGreen, StrokeWidth = 3 } },
                { "13", new SKPaint { Color = SKColors.MidnightBlue, StrokeWidth = 3 } },
                { "14", new SKPaint { Color = SKColors.MediumVioletRed, StrokeWidth = 3 } },
                { "7bis", new SKPaint { Color = SKColors.LightBlue, StrokeWidth = 3 } }
            };
            ...
            canvas.DrawText(titre, width / 2, 40, titlePaint);
            ...
            for (int i = 0; i < chemin.Count - 1; i++)
            {
                ...
                var paint = couleurParLigne.ContainsKey(n1.Ligne) ? couleurParLigne[n1.Ligne] : new SKPaint { Color = SKColors.Black, StrokeWidth = 3 };
                canvas.DrawLine(p1, p2, paint);

                double temps = graph.Distance(n1, n2) / 25.0 * 60.0;
                string label = $"{Math.Round(temps, 1)} min";
                canvas.DrawText(label, (p1.X + p2.X) / 2, (p1.Y + p2.Y) / 2, textPaint);
            }

            foreach (var noeud in chemin)
            {
                var pos = positions[noeud.Id];
                canvas.DrawCircle(pos, 6, new SKPaint { Color = SKColors.DarkSalmon });
                canvas.DrawText($"{noeud.Nom} ({noeud.Ligne})", pos.X + 6, pos.Y - 6, textPaint);
            }

            using var image = surface.Snapshot();
            using var data = image.Encode(SKEncodedImageFormat.Png, 100);
            string cheminFichier = "chemin_court_skia.png";
        ...
        /// <summary>
        /// Créer un dictionnaire qui associe à chaque id dune station une liste de noeuds correspondant aux stations qui la précède 
        /// </summary>
